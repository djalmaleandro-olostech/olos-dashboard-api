...
/**
 * @OA\Info(title="Documentação  API", version="1.0")
 */
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        if (!PermissionService::isDevUser() && !PermissionService::isAdminUser()) {
            $query->whereDoesntHave('roles', function ($roleQuery) {
                $roleQuery->whereIn('roles.id', [1, 2]);
            });
        } elseif (!PermissionService::isDevUser() && PermissionService::isAdminUser()) {
            $query->whereDoesntHave('roles', function ($roleQuery) {
